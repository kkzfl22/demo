...
public class TestMain
{
	public static void main(String[] args)
	{
		...
		financeDep.add(j);
		// 打印写完的树状结构
		System.out.print(ceo.getInfo());
		// 打印出来整个树形
		getAllSubordinateInfo(ceo.getSubordinateInfo(),1);
	}

	// 遍历所有的树枝节点，打印出信息
	@SuppressWarnings({ "unused", "rawtypes" })
	private static void getAllSubordinateInfo(List subordinateList,int i)
	{
		...
		for (int m = 0; m < length; m++)
		{
			...
			{ // 是个叶子节点，也就是员工
				...
				System.out.print(((Leaf) s).getInfo());
			} else
			{
				...
				System.out.print(branch.getInfo());
				i+=1;
				// 再递归调用
				getAllSubordinateInfo(branch.getSubordinateInfo(),i);
				i= i-1;
			}
